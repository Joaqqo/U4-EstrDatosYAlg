...
class ABB:
    ...
    def Cuenta(self,raiz): #Cuenta la cantidad de nodos que hay.
        if raiz is not None:
            return 1 + self.Cuenta(raiz.getIzquierda()) + self.Cuenta(raiz.getDerecha())
        else:
            return 0

    def Sucesores(self, raiz):
        if raiz is not None:
            print(raiz.getClave(), end="- ")
            self.Sucesores(raiz.getIzquierda())
            self.Sucesores(raiz.getDerecha())
